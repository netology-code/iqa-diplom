...
- Для заданий 1, 2.1, 3.1: доработанную версию надо создать в новом листе на изначальной таблице. Для этого мы [переименовываем](https://prnt.sc/11r2ogg) уже существующий лист в "ДЗ Версия 1", создаем новый через [дублирование](https://prnt.sc/11r2tlh), новый лист переименовываем в "ДЗ Версия 2" и выполняем доработки уже в ней. Аналогично для доработок после второй. 
...
* Мы ожидаем от вас список проверок функционала личного кабинета без учета хедера и футера страницы, то есть то, что есть в этой [области](https://prnt.sc/12h7mqr).
